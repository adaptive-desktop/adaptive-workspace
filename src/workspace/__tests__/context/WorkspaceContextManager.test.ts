...
describe('WorkspaceContextManager', () => {
  ...
  it('should include a viewportStates property in the context', () => {
    const context = manager.getContext(laptopBounds);
    expect(context).toHaveProperty('viewportStates');
    expect(Array.isArray(context.viewportStates)).toBe(true);
  });
